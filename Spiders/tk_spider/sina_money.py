...
def get_s_money():
    ...
    url_list = res_text.xpath("//div[@class='fin_tabs0_c0']//a/@href")
    print(url_list)
    print('一共', len(url_list))
    for x in url_list:
        if 'zhibo' in x or 'zr_d' in x:
            pass
        else:
            try:
            ...
            try:
                ...
                if item['source'] == '':
                    item['source'] = ''.join(res2_text.xpath("//div[@class='date-source']/span[@class='source ent-source']/text()"))
                item['url'] = x
