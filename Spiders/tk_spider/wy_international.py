...
def get_wy_inter():
    url = 'https://temp.163.com/special/00804KVA/cm_guoji.js?callback=data_callback'
    headers = {
    ...
    # data = re.findall(r'.*data_callback(.*?)政府工', data)
    print(data)

    for x in data:
        if 'photoview' not in x['docurl']:
            try:
            ...
            if item['title'] == '':
                item['title'] = ''.join(res2_text.xpath("//div[@class='article_title']/h2/text()")).replace('\r','').replace('\n','').replace(
            ...
            try:
                ...
                item['date'] = ''.join(res2_text.xpath("//p[@class='time']/span[1]/text()"))
                item['time'] = ''


            item['source'] = ''.join(res2_text.xpath("//a[@id='ne_article_source']/text()"))
            if item['source'] == '':
                item['source'] = ''.join(res2_text.xpath("//p[@class='time']/span[3]/text()"))

            item['url'] = x['docurl']
            item['content'] = ''.join(res2_text.xpath("//div[@id='endText']/p/text()")).replace('\r','').replace('\n','').replace(
                '\t', '').replace('\u3000', '').replace('  ', '').replace('\xa0 ', '')
            if item['content'] == '':
                item['content'] = ''.join(res2_text.xpath("//div[@class='content']/p/text()")).replace('\r','').replace('\n','').replace(
                '\t', '').replace('\u3000', '').replace('  ', '').replace('\xa0 ', '')
